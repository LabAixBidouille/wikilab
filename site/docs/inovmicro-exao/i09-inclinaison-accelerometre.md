...
- Initialiser un capteur I2C avec le module `ism330dl`
- Lire l'accélération sur les axes X, Y et Z en unités G
- Utiliser la valeur absolue pour ignorer le sens de l'accélération
- Détecter l'orientation de la carte avec la méthode `orientation()`
- Afficher des données capteur en temps réel sur l'écran OLED avec `steami_screen`
---
...
- **Boucle principale** : lit les trois axes, calcule la **norme** du vecteur accélération (`√(ax² + ay² + az²)`). Au repos, cette norme vaut toujours environ 1 G (la gravité). Si elle dépasse `SEUIL_CHOC`, c'est qu'un choc ou une secousse s'est produit — le buzzer sonne.
...
Plutôt que des chiffres bruts, on peut représenter l'inclinaison visuellement avec `screen.bar()`. Par exemple, mapper l'axe X (−1 G … +1 G) sur une barre de progression (0 … 100) :

```python
def axe_vers_pct(valeur_g):
    """Convertit une valeur en G (−1 à +1) en pourcentage (0 à 100)."""
    clamp = max(-1.0, min(1.0, valeur_g))
    return int((clamp + 1.0) * 50)   # −1G → 0%, 0G → 50%, +1G → 100%
...
- [Accéléromètre (Wikipédia)](https://fr.wikipedia.org/wiki/Acc%C3%A9l%C3%A9rom%C3%A8tre) : principes physiques et applications.
...
- [Détection de chute libre avec accéléromètre (Hackster)](https://www.hackster.io/RVLAD/free-fall-detection-using-3-axis-accelerometer-06383e) : méthode pour déterminer la chute libre avec un accéléromètre 3 axes.
---

_Cette fiche fait partie du projet [I-Novmicro #2 : Action EXAO](/projets/inovmicro-exao). Adaptée de la fiche Let's STEAM [`r1as09-accelerometre`](/ressources/lets-steam/r1as09-accelerometre) sous licence [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/deed.fr)._
